...
int select_menu[10] = { 0 };  

int select_option[10] = { 0 };  //유료 옵션 선택의 리턴값을 저장하는 배열
int option[2][10] = {}; // 1행에 무료옵션 배열, 2행에 유료옵션 배열 저장//값이 0이면 옵션x, 값이 1 이면 옵션 o
int price_option[10] = { 0 }; //option의 가격을 저장하는 배열

int main_price[4];   //main메뉴의 가격을 저장하는 배열
int sum_price[10];     
int sum_menu[1];   //main 메뉴의 번호를 저장하는 배열
...
int main() {
    ...
    do {
        ...
        if (select_menu[count_menu] != 0) { //결제 하기 아니라면
            ...
            sum_price[num] = receipt.price_total(price, option); //총 금액을 계산하는 함수(price값에 option값을 더한다.)
